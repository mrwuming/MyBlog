...
// 加载配置文件
var objConfig = settings.log4jsConfig;
...
logger.writeDebug = function(msg){
    if(msg == null)
        msg = "";
    logDebug.debug(msg);
};

logger.writeInfo = function(msg){
    if(msg == null)
        msg = "";
    logInfo.info(msg);
};

logger.writeWarn = function(msg){
    if(msg == null)
        msg = "";
    logWarn.warn(msg);
};

logger.writeErr = function(msg, exp){
    if(msg == null)
        msg = "";
    if(exp != null)
        msg += "\r\n" + exp;
...
function checkAndCreateDir(dir){
    ...
    var fatherDir=dir.substr(0,dir.lastIndexOf(path.sep));
    if(fs.existsSync(fatherDir)){
        fs.mkdirSync(dir);
    }else{
        checkAndCreateDir(fatherDir);
        fs.mkdirSync(dir);
    }
}
